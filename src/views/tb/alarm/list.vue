      ...
      <template #tableTitle>
        <div class="space-x-2">
          ...
          <a-input
            ...
            :placeholder="t('alarmPage.searchPlaceholder')"
            allow-clear
            @change="reload"
            style="width: 240px"
          >
      ...
      <template #severity="{ record }">
        <span v-if="record.severity">
          <Tag :color="ALARM_SEVERITY_OPTIONS.find((item) => item.value === record.severity)?.color">
            {{ t('alarmSeverity.' + record.severity) }}
          </Tag>
        </span>
      </template>
    </BasicTable>
...
<script lang="ts" setup>
  import { defineComponent, reactive, ref, computed, type PropType } from 'vue';
  ...
  const rangePresets = computed(() => [
    { label: t('alarmPage.preset.today'), value: [dayjs().startOf('D'), dayjs()] },
    { label: t('alarmPage.preset.last1h'), value: [dayjs().subtract(1, 'hour'), dayjs()] },
    { label: t('alarmPage.preset.last6h'), value: [dayjs().subtract(6, 'hour'), dayjs()] },
    { label: t('alarmPage.preset.last1d'), value: [dayjs().subtract(1, 'day').startOf('D'), dayjs()] },
    { label: t('alarmPage.preset.last3d'), value: [dayjs().subtract(2, 'day').startOf('D'), dayjs()] },
    { label: t('alarmPage.preset.last7d'), value: [dayjs().subtract(6, 'day').startOf('D'), dayjs()] },
  ]);
  ...
  const tableColumns: BasicColumn[] = [
    ...
    {
      title: t('alarmPage.assignee'),
      dataIndex: 'assignee.firstName',
      key: 'assignee.firstName',
      align: 'center',
    },
    {
      title: t('alarmPage.severity'),
      dataIndex: 'severity',
      key: 'severityList',
      align: 'center',
      width: 130,
      filters: ALARM_SEVERITY_OPTIONS.map((item) => ({ text: t('alarmSeverity.' + item.value), value: item.value })),
      slot: 'severity',
    },
    {
      title: t('alarmPage.status'),
      dataIndex: 'status',
      key: 'statusList',
      align: 'center',
      width: 130,
      filters: ALARM_STATUS_OPTIONS.map((item) => ({ text: t('alarmStatus.' + item.value), value: item.value })),
      format: (text: any) => (text ? t('alarmStatus.' + text) : ''),
    },
    {
      title: t('alarmPage.createdTime'),
    ...
    actions: (record: Recordable) => [
      {
        ...
        title: t('alarmPage.detail'),
        onClick: handleDetail.bind(this, { ...record }),
      },
      {
        icon: 'ant-design:delete-outlined',
        color: 'error',
        title: t('alarmPage.deleteLabel'),
  ...
  async function handleDelete(record: Recordable) {
    createConfirm({
      iconType: 'error',
      title: t('alarmPage.delete.confirmTitle', { name: record.name }),
      content: t('alarmPage.delete.confirmContent'),
      centered: false,
      okText: t('alarmPage.delete.okText'),
      okButtonProps: {
        type: 'primary',
        danger: true,
      },
      onOk: async () => {
        try {
          ...
          showMessage(t('alarmPage.delete.success'));
        } catch (error: any) {
          console.log(error);
        } finally {
          handleSuccess();
