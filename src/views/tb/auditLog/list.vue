  ...
  <div class="audit-log-list">
    <BasicTable @register="registerTable">
      ...
      <template #actionStatus="{ record }">
        <Tag color="success" v-if="record.actionStatus == 'SUCCESS'">{{ t('auditLogPage.status.success') }}</Tag>
        <Tag color="error" v-if="record.actionStatus == 'FAILURE'">{{ t('auditLogPage.status.failure') }}</Tag>
      </template>
    </BasicTable>
...
<script lang="ts" setup>
  ...
  const getTitle = {
    value: router.currentRoute.value.meta.title || 'auditLog',
  };

  const rangePresets = computed(() => [
    { label: t('auditLogPage.preset.today'), value: [dayjs().startOf('D'), dayjs()] },
    { label: t('auditLogPage.preset.last1h'), value: [dayjs().subtract(1, 'hour'), dayjs()] },
    { label: t('auditLogPage.preset.last6h'), value: [dayjs().subtract(6, 'hour'), dayjs()] },
    { label: t('auditLogPage.preset.last1d'), value: [dayjs().subtract(1, 'day').startOf('D'), dayjs()] },
    { label: t('auditLogPage.preset.last3d'), value: [dayjs().subtract(2, 'day').startOf('D'), dayjs()] },
    { label: t('auditLogPage.preset.last7d'), value: [dayjs().subtract(6, 'day').startOf('D'), dayjs()] },
  ]);
  ...
  const tableColumns: BasicColumn[] = [
    ...
    {
      title: t('auditLogPage.entityType'),
      dataIndex: 'entityId.entityType',
      key: 'entityId.entityType',
      align: 'left',
      width: 160,
      ifShow: isEmpty(props.entityId),
      format: (text: any) => (text ? t('entityType.' + text) : ''),
    },
    {
      title: t('auditLogPage.entityName'),
    ...
    {
      title: t('auditLogPage.actionType'),
      dataIndex: 'actionType',
      key: 'actionTypes',
      align: 'left',
      width: 160,
      filters: ACTION_TYPE_OPTIONS.map((item) => ({ text: t('actionType.' + item.value), value: item.value })),
      format: (text: any) => (text ? t('actionType.' + text) : ''),
    },
    {
      title: t('auditLogPage.status.title'),
