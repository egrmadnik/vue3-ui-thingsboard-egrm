    ...
    <BasicTable @register="registerTable">
      ...
      <template #tableTitle>
        <div class="space-x-2">
          ...
          >
            <Icon icon="i-fluent:add-12-filled" /> {{ t('devicePage.addDevice') }}
          </a-button>
          <a-input
            v-model:value="searchParam.textSearch"
            :placeholder="t('devicePage.searchPlaceholder')"
            allow-clear
            @change="reload"
            style="width: 240px"
          >
      ...
      <template #active="{ record }">
        <Tag v-if="record.active == true" color="success">{{ t('devicePage.status.online') }}</Tag>
        <Tag v-if="record.active == false" color="error">{{ t('devicePage.status.offline') }}</Tag>
      </template>
...
<script lang="ts" setup>
  ...
  const tableColumns: BasicColumn[] = [
    ...
    },
    {
      ...
      filterMultiple: false,
      filters: [
        { text: t('devicePage.status.online'), value: 'true' },
        { text: t('devicePage.status.offline'), value: 'false' },
      ],
    },
    {
      title: t('devicePage.public'),
    ...
    {
      title: t('devicePage.createdTime'),
  ...
  async function handleDelete(record: Recordable) {
    createConfirm({
      iconType: 'error',
      title: t('devicePage.delete.confirmTitle', { name: record.name }),
      content: t('devicePage.delete.confirmContent'),
      centered: false,
      okText: t('devicePage.delete.okText'),
      okButtonProps: {
        type: 'primary',
        danger: true,
      },
      onOk: async () => {
        try {
          ...
          showMessage(t('devicePage.delete.success'));
        } catch (error: any) {
          console.log(error);
        } finally {
          handleSuccess();
  ...
  function handleAssignToPublic(record: Recordable) {
    createConfirm({
      iconType: 'info',
      title: t('devicePage.publicConfirm.title', { name: record.name }),
      content: t('devicePage.publicConfirm.content'),
      centered: false,
      okText: t('devicePage.publicConfirm.okText'),
      okButtonProps: {
        type: 'primary',
      },
      onOk: async () => {
        try {
          ...
          showMessage(t('devicePage.publicConfirm.success'));
        } catch (error: any) {
          console.log(error);
        } finally {
          handleSuccess();
  ...
  function handleUnAssignFromCustomer(record: Recordable) {
    createConfirm({
      iconType: 'info',
      title: t('devicePage.privateConfirm.title', { name: record.name }),
      content: t('devicePage.privateConfirm.content'),
      centered: false,
      okText: t('devicePage.privateConfirm.okText'),
      okButtonProps: {
        type: 'primary',
      },
      onOk: async () => {
        try {
          ...
          showMessage(t('devicePage.privateConfirm.success'));
        } catch (error: any) {
          console.log(error);
        } finally {
          handleSuccess();
