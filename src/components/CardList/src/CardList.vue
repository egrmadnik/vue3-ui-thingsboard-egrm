    ...
    >
      ...
      <template #renderItem="{ item }">
        <ListItem style="padding: 10px; margin: 10px 0 0">
          <Card>
            <template #actions>
              <EditOutlined @click="showMessage(t('component.cardList.editClicked'))" />
              <Dropdown
                :trigger="['hover']"
                :dropMenuList="[
                  {
                    text: t('component.cardList.deleteLabel'),
                    event: '1',
                    popConfirm: {
                      title: t('component.cardList.confirmDelete'),
...
<script lang="ts" setup>
  ...
  const [registerForm, { validate }] = useForm({
    schemas: [{ field: 'loginCode', component: 'Input', label: t('component.cardList.account') }],
    labelWidth: 80,
  ...
  const paginationProp = ref({
    ...
    current: page,
    total,
    showTotal: (total: number) => t('component.cardList.total', { total }),
    onChange: pageChange,
